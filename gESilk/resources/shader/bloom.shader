...
void main()
{
    ...
    vec2 texSize = vec2(textureSize(u_Texture, int(LodAndMode.x)));
    vec4 color = vec4(1, 0, 1, 1);
    if (uint(LodAndMode.y) == BLOOM_MODE_PREFILTER)
    {
        color.rgb = DownsampleBox13(u_Texture, 0, texCoords, 1.0f / texSize);
        color = Prefilter(color, texCoords);
        color.a = 1.0f;
    }
    else if (uint(LodAndMode.y) == BLOOM_MODE_UPSAMPLE_FIRST)
    {
        vec2 bloomTexSize = vec2(textureSize(u_Texture, int(LodAndMode.x + 1.0f)));
        float sampleScale = 1.0f;
        vec3 upsampledTexture = UpsampleTent9(u_Texture, LodAndMode.x + 1.0f, texCoords, 1.0f / bloomTexSize, sampleScale);

        vec3 existing = textureLod(u_Texture, texCoords, LodAndMode.x).rgb;
        color.rgb = existing + upsampledTexture;
    }
    else if (uint(LodAndMode.y) == BLOOM_MODE_UPSAMPLE)
    {
        vec2 bloomTexSize = vec2(textureSize(u_BloomTexture, int(LodAndMode.x + 1.0f)));
        float sampleScale = 1.0f;
        vec3 upsampledTexture = UpsampleTent9(u_BloomTexture, LodAndMode.x + 1.0f, texCoords, 1.0f / bloomTexSize, sampleScale);

        vec3 existing = textureLod(u_Texture, texCoords, LodAndMode.x).rgb;
        color.rgb = existing + upsampledTexture;
    }
    else if (uint(LodAndMode.y) == BLOOM_MODE_DOWNSAMPLE)
    {
        // Downsample
        color.rgb = DownsampleBox13(u_Texture, LodAndMode.x, texCoords, 1.0f / texSize);
    }
